...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        evl = scheme_eval(first, env)
        if not validate_procedure(evl):
            return scheme_apply(evl, (rest.map(lambda x: scheme_eval(x, env))), env)
        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return '#t'
    evled = scheme_eval(expressions.first, env)
    if expressions.rest == nil:
        return evled
    elif is_false_primitive(evled):
        return evled
    else:
